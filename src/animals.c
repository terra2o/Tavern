...
void cats_tick(Animals* a, MessageLog* log)
{
    ...
    int fertile_pairs = mature_males < mature_females ? mature_males : mature_females;
    int births = 0;
    for (int i = 0; i < fertile_pairs; i++) {
    ...
    if (births > 0) {
        char buf[128];
        snprintf(buf, sizeof(buf), "%d new kitten%s born.", births, births == 1 ? " was" : "s were");
        log_message(log, buf, LOG_INFO);
    }
    if (deaths > 0) {
        char buf[128];
        snprintf(buf, sizeof(buf), "%d cats died of old age.", deaths);
        log_message(log, buf, LOG_INFO);
    }
}
